...
const AdminSessionController = {
  fetchRescheduleRequests: async (req: Request, res: Response) => {
    try {
      ...
      return res
        .status(200)
        .json(new ApiResponse(true, 200, "Rescheduled bookings fetched successfully", bookings));
    } catch (error) {
  ...
  processRescheduleRequest: async (req: Request, res: Response) => {
    try {
          ...
          return res
            ...
            .json(
              new ApiResponse(
                false,
                400,
                "New slot ID is required for approving reschedule request"
              )
            );
        }
      }
      const processedAction = await AdminSessionService.processRescheduleRequest(
        bookingId,
        newSlotId,
        req.user.id,
        action
      ...
      return res
        .status(200)
        .json(
          new ApiResponse(true, 200, "Rescheduled bookings processed successfully", processedAction)
        );
    } catch (error) {
  ...
  cancelBooking: async (req: Request, res: Response) => {
    try {
      ...
      await AdminSessionService.cancelBooking(bookingId, reason);

      return res.status(200).json(new ApiResponse(true, 200, "Booking cancelled successfully"));
    } catch (error) {
