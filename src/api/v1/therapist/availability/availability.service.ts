...
export class AvailabilityService {
      ...
      for (const day of availabilities) {
        ...
        for (const { startTime, endTime } of intervals) {
          ...
          if (!this.isEndTimeAfterStartTime(startTime, endTime)) {
            throw new ApiError(
              400,
              `End time ${endTime} must be after start time ${startTime} for ${dayOfWeek}`
            );
          }
  ...
  async updateAvailability(
    ...
    try {
      ...
      if (
        input.startTime &&
        input.endTime &&
        !this.isEndTimeAfterStartTime(input.startTime, input.endTime)
      ) {
        throw new ApiError(400, "End time must be after start time");
      }
      ...
      const effectiveToDate = input.effectiveTo
        ? new Date(input.effectiveTo as any)
        : (old.effectiveTo ?? null);
      ...
      return await prisma
        ...
        .then(async (jobPayload: UpdateAvailabilityJobPayload) => {
          ...
          const created = await prisma.therapistAvailability.findUnique({
            where: { id: jobPayload.newAvailabilityId },
          });
          return created;
        })
        .catch((err: unknown) => {
  ...
  private async checkOverlappingAvailability(
    ...
    const andConditions: any[] = [{ startTime: { lt: endTime } }, { endTime: { gt: startTime } }];

    // Exclude expired availability records (effectiveTo in the past)
    // and ensure effective date ranges actually overlap
    andConditions.push({
  ...
  private async filterExistingSlots(slots: any[]) {
    ...
    const existingSet = new Set(
      existing.map(
        (e: (typeof existing)[number]) =>
          e.therapistId + e.startDateTime.toISOString() + e.endDateTime.toISOString()
      )
    );
    ...
    return slots.filter(
      (s) =>
        !existingSet.has(
          s.therapistId + s.startDateTime.toISOString() + s.endDateTime.toISOString()
        )
    );
  }
