...
import {
  BillingCycle,
  ConfirmOrgPaymentDTO,
  CreateCustomPlanDTO,
  createPaymentLinkDTO,
  UpdateCustomPlanDTO,
} from "./custom-plan.dto";
import {
  emailFromAddress,
  organizationOnboardingSubjects,
} from "../../../../../shared/config/email.config";
import {
  customPlanPaymentLinkTemplate,
  orgActivationTemplate,
} from "../../../../../shared/email-templates/organizations/custom-plan";

import { addMonths, addYears, addDays } from "date-fns";
import { randomUUID } from "crypto";

export const calculateValidTill = (from: Date, billingCycle: string): Date => {
  switch (billingCycle) {
    case "MONTHLY":
...
const CustomPlanService = {
  ...
  sendPaymentLink: async (orgId: string, data: createPaymentLinkDTO) => {
    ...
    const accountTeamEmail = data.orgAccountTeamContactEmail ?? plan.org.orgAccountTeamContactEmail;

    const accountTeamName = data.orgAccountTeamContactName ?? plan.org.orgAccountTeamContactName;
    ...
    const recipientFirstName = hasAccountTeam ? accountTeamName! : plan.org.contactName;
  ...
  recordPayment: async (orgId: string, data: ConfirmOrgPaymentDTO, adminId: string) => {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      await tx.organization.update({
        where: { id: orgId },
        data: { status: "ACTIVE", setupToken, setupTokenExpiresAt: addDays(now, 7) },
      });
