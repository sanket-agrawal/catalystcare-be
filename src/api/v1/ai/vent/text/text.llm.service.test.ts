...
describe("VentLLMService", () => {
  ...
  describe("processVentMessage", () => {
    it("should process user message and return parsed LLM response", async () => {
      ...
      expect(callArgs.messages[2]).toEqual({ role: "user", content: "I am feeling stressed today." });
    ...
    it("should handle error in callGroq and return a safe fallback response", async () => {
      ...
      expect(response).toEqual({
        valid: true,
        reply: "I'm here with you. Sometimes putting feelings into words is the hardest part. Would you like to share a little more about what's going on?",
      });
    });
