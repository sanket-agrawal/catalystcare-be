...
import { notifyPendingSlotSubscribersService } from "../../website/website.service";
...
function generateSlotsForInterval(
  dayDate: Date,
  startTime: string,
  endTime: string,
  durationMin: number
  ...
  async generateSlots(payload: {
    ...
    for (const d of dates) {
      ...
      for (const row of rowsForDay) {
        ...
        const intervalSlots = generateSlotsForInterval(
          d,
          row.startTime,
          row.endTime,
          row.slotDuration
    ...
    if (createdCount > 0) {
      notifyPendingSlotSubscribersService(therapistId).catch((err) => {
        console.error("Error triggering slot notifications for therapist", therapistId, err);
      });
    }
