...
describe("Therapist Controller", () => {
  ...
  describe("getUpcomingBookings", () => {
    it("should fetch upcoming 7-day bookings successfully", async () => {
      const mockBookings = [
        {
          id: "booking-1",
          sessionNotes: "Need help managing stress",
          client: {
            id: "client-1",
            name: "John Doe",
            aiSummary: "Client reports severe work stress",
            latestAssessment: { recentFeeling: "Anxious" },
          },
        },
      ];
      (getUpcoming7DaysTherapistBookings as any).mockResolvedValue(mockBookings);

      await therapistController.getUpcomingBookings(mockReq as Request, mockRes as Response);

      expect(getUpcoming7DaysTherapistBookings).toHaveBeenCalledWith("therapist-profile-1");
      expect(mockRes.status).toHaveBeenCalledWith(200);
      expect(ApiResponse).toHaveBeenCalledWith(
        true,
        200,
    ...
    it("should handle ApiError in getUpcomingBookings", async () => {
      const apiError = new ApiError(404, "Not found");
      (getUpcoming7DaysTherapistBookings as any).mockRejectedValue(apiError);
  ...
  describe("getBookingDetails", () => {
    it("should fetch booking details including assessment, session notes, and AI summary", async () => {
      const mockDetail = {
        id: "booking-1",
        sessionNotes: "First session goal: work-life balance",
        client: {
          id: "client-1",
          name: "John Doe",
          aiSummary: "Client summary generated by AI",
          latestAssessment: {
            recentFeeling: "Overwhelmed",
            nightSleep: "Poor",
          },
        },
      };
      (getBookingDetailsForTherapist as any).mockResolvedValue(mockDetail);

      mockReq.params = { bookingId: "booking-1" };

      await therapistController.getBookingDetails(mockReq as Request, mockRes as Response);

      expect(getBookingDetailsForTherapist).toHaveBeenCalledWith(
        "booking-1",
        "therapist-profile-1"
      );
      expect(mockRes.status).toHaveBeenCalledWith(200);
    ...
    it("should handle ApiError in getBookingDetails", async () => {
      const apiError = new ApiError(404, "Booking not found");
      (getBookingDetailsForTherapist as any).mockRejectedValue(apiError);
  ...
  describe("assignHomework", () => {
    it("should assign homework successfully", async () => {
      const mockResult = { id: "booking-1", homework: "mindfulness" };
      (therapistService.assignHomework as any).mockResolvedValue(mockResult);

      mockReq.params = { bookingId: "booking-1" };
      mockReq.body = { homework: "mindfulness" };

      await therapistController.assignHomework(mockReq as Request, mockRes as Response);

      expect(therapistService.assignHomework).toHaveBeenCalledWith(
        "therapist-profile-1",
        "booking-1",
        "mindfulness"
      );
    ...
    it("should throw 400 if homework is missing", async () => {
      mockReq.params = { bookingId: "booking-1" };
      mockReq.body = {};
      ...
      expect(mockRes.status).toHaveBeenCalledWith(400);
      expect(ApiResponse).toHaveBeenCalledWith(false, 400, "Homework description is required");
    });

    it("should handle ApiError in assignHomework", async () => {
      const apiError = new ApiError(404, "Booking not found");
      (therapistService.assignHomework as any).mockRejectedValue(apiError);
