...
export const adminController = {
      ...
      res
        ...
        .json(
          new ApiResponse(
            true,
            200,
            `Therapist profile ${approve ? "approved" : "rejected"} successfully`,
            updatedProfile
          )
        );
    } catch (error) {
  ...
  adminLogin: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(new ApiResponse(success, success ? 200 : 400, "OTP Sent to ADMINs", success));
    } catch (error) {
  ...
  adminVerifyLoginOtp: async (req: Request, res: Response) => {
    try {
      ...
      const result = await adminService.verifyAdminLoginOTP(email, otp);

      setRefreshTokenCookie(res, result.refreshToken);

      res
        .status(200)
        .json(new ApiResponse(true, 200, "Admin Login Success", { token: result.token }));
    } catch (error) {
  ...
  createComissionRate: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(new ApiResponse(true, 201, "Commission Rate Created Successfully", commissionRate));
    } catch (error) {
  ...
  fetchAllCommissionRate: async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json(new ApiResponse(true, 200, "Comission Rates Fetched Success", rates));
    } catch (error) {
  ...
  dashboard: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(new ApiResponse(true, 201, "Dashboard Fetched Successfully", dashboardData));
    } catch (error) {
  ...
  billingDashboard: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(new ApiResponse(true, 201, "Billing Dashboard Fetched Successfully", billingData));
    } catch (error) {
  ...
  fetchTherapistVPA: async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json(new ApiResponse(true, 200, "Therapist VPA Fetched Success", rawVPA));
    } catch (error) {
  ...
  fetchApprovedTherapist: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(new ApiResponse(true, 200, "Approved Therapist Fetched Success", therapist));
    } catch (error) {
  ...
  fetchEmailBlastLogs: async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json(new ApiResponse(true, 200, "Email Blast Logs Fetched Success", logs));
    } catch (error) {
  ...
  putTherapistProfileOnHold: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(
          new ApiResponse(true, 200, `Therapist profile put on hold successfully`, updatedProfile)
        );
    } catch (error) {
  ...
  removeTherapistProfileHold: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(
          new ApiResponse(true, 200, `Therapist profile hold removed successfully`, updatedProfile)
        );
    } catch (error) {
  ...
  programBillingDashboard: async (req: Request, res: Response) => {
    try {
      const billingData = await adminService.programBillingsDashboard();
      res
        .status(200)
        .json(
          new ApiResponse(true, 200, "Programs Billing Dashboard Fetched Successfully", billingData)
        );
    } catch (error) {
      console.log("Error fetching program billing dashboard", error);
      if (error instanceof ApiError) {
        return res
          .status(error.statusCode)
          .json(new ApiResponse(false, error.statusCode, error.message));
  ...
  adminLogout: async (req: Request, res: Response) => {
    try {
      ...
      if (token) {
        await adminService.adminLogout(token);
      }
      clearRefreshTokenCookie(res);
      res.status(200).json(new ApiResponse(true, 200, "Admin logged out successfully"));
    } catch (error) {
      clearRefreshTokenCookie(res);
      res.status(200).json(new ApiResponse(true, 200, "Admin logged out successfully"));
    }
