...
import {
  therapistProfileApprovalTemplate,
  therapistProfileHoldRemovedTemplate,
  therapistProfileOnHoldTemplate,
...
export const adminService = {
  ...
  approveRejectTherapistProfile: async (profileId: string, approve: boolean) => {
    try {
      ...
      if (profile.status !== "PENDING")
        throw new ApiError(400, "Therapist profile already reviewed");
      const data: {
        status: TherapistProfileStatus;
        approvedAt?: Date;
        rejectionReason?: string | null;
      } = { status: approve ? TherapistProfileStatus.APPROVED : TherapistProfileStatus.REJECTED };
      if (approve) {
      ...
      await emailQueue.add("sendOtp", {
        ...
        subject: approve
          ? emailSubjects().therapistOnboarding
          : emailSubjects().therapistOnboarding,
  ...
  dashboardService: async () => {
    try {
      ...
      const [therapistCount, clientCount, todaysSessions, todayRevenueResult] = await Promise.all([
