...
const ProgramController = {
  createProgram: async (req: Request, res: Response) => {
    try {
      ...
      res.status(201).json(new ApiResponse(true, 201, "Program Created Successfully", program));
    } catch (error) {
  ...
  fetchAllPrograms: async (req: Request, res: Response) => {
    try {
      ...
      res.status(201).json(new ApiResponse(true, 201, "Program Fetched Successfully", programs));
    } catch (error) {
  ...
  updateProgram: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(new ApiResponse(true, 200, "Program Updated Successfully", updatedProgram));
    } catch (error) {
  ...
  publishProgram: async (req: Request, res: Response) => {
    try {
      const publishedProgram = await ProgramService.publishProgram(
        req.params.programId,
        req.user.therapistProfileId
      );
      res
        .status(200)
        .json(new ApiResponse(true, 200, "Program Published Successfully", publishedProgram));
    } catch (error) {
  ...
  unPublishProgram: async (req: Request, res: Response) => {
    try {
      const unPublishedProgram = await ProgramService.unPublishProgram(
        req.params.programId,
        req.user.therapistProfileId
      );
      res
        .status(200)
        .json(new ApiResponse(true, 200, "Program Un-Published Successfully", unPublishedProgram));
    } catch (error) {
  ...
  addPlanToProgram: async (req: Request, res: Response) => {
    try {
      const plan = await ProgramService.addPlanToProgram(
        req.params.programId,
        req.user.therapistProfileId,
        req.body
      );
      res.status(200).json(new ApiResponse(true, 200, "Plan Added to Program Successfully", plan));
    } catch (error) {
  ...
  publishPlan: async (req: Request, res: Response) => {
    try {
      ...
      res.status(200).json(new ApiResponse(true, 200, "Plan Published Successfully", plan));
    } catch (error) {
      console.log("Error in Add Plan To Program", error);
      if (error instanceof ApiError) {
        res.status(error.statusCode).json(new ApiResponse(false, error.statusCode, error.message));
  ...
  unPublishPlan: async (req: Request, res: Response) => {
    try {
      const plan = await ProgramService.unPublishPlan(
        req.params.planId,
        req.user.therapistProfileId
      );
      res.status(200).json(new ApiResponse(true, 200, "Plan Un-Published Successfully", plan));
    } catch (error) {
  ...
  updatePlan: async (req: Request, res: Response) => {
    try {
      const updatedPlan = await ProgramService.updatePlan(
        req.params.planId,
        req.user.therapistProfileId,
        req.body
      );
      res.status(200).json(new ApiResponse(true, 200, "Plan Updated Successfully", updatedPlan));
    } catch (error) {
      console.log("Error in Update Plan", error);
      if (error instanceof ApiError) {
        res.status(error.statusCode).json(new ApiResponse(false, error.statusCode, error.message));
      } else {
        res.status(400).json(new ApiResponse(false, 400, "Something went wrong"));
  ...
  fetchProgramBookings: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(new ApiResponse(true, 200, "Program Bookings fetched Successfully", plan));
    } catch (error) {
  ...
  fetchPurchaseById: async (req: Request, res: Response) => {
    try {
      ...
      res
        .status(200)
        .json(new ApiResponse(true, 200, "Purchase Bookings fetched Successfully", plan));
    } catch (error) {
